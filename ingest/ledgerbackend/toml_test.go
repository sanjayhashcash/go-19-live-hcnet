	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			expectedError: "invalid captive core toml: NETWORK_PASSPHRASE in captive core config file: " +
				"Public Global Hcnet Network ; September 2015 does not match Aurora network-passphrase " +
		...
		},
		{
			...
			expectedError: "invalid captive core toml: found duplicate home domain in captive " +
				"core configuration: testnet.hcnet.org",
		},
		{
		...
		},
		{
			...
			logPath:           nil,
			expectedError: "invalid captive core toml: found invalid home domain entry which is " +
				"missing a QUALITY value: testnet.hcnet.org",
		},
		{
			...
			expectedError: "invalid captive core toml: found invalid home domain entry which has an " +
				"invalid QUALITY value: testnet.hcnet.org",
		},
		{
...
func TestGenerateConfig(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			mode:         hcnetCoreRunnerModeOffline,
			appendPath:   "",
		...
		},
		{
			...
			httpPort:     newUint(6789),
			peerPort:     nil,
			logPath:      newString("/var/hcnet-core/test.log"),
		},
		{
			name:         "online config with appendix",
			mode:         hcnetCoreRunnerModeOnline,
...
func TestExternalStorageConfigUsesDatabaseToml(t *testing.T) {
	...
	captiveCoreToml, err = NewCaptiveCoreToml(params)
	assert.NoError(t, err)
	captiveCoreToml.Database = "sqlite3:///etc/defaults/hcnet.db"

	configBytes, err := generateConfig(captiveCoreToml, hcnetCoreRunnerModeOffline)

	assert.NoError(t, err)
	toml := CaptiveCoreToml{}
	require.NoError(t, toml.unmarshal(configBytes, true))
	assert.Equal(t, toml.Database, "sqlite3:///etc/defaults/hcnet.db")
}
...
func TestDBConfigDefaultsToSqlite(t *testing.T) {
	...
	toml := CaptiveCoreToml{}
	require.NoError(t, toml.unmarshal(configBytes, true))
	assert.Equal(t, toml.Database, "sqlite3://hcnet.db")
}
