...
func TestDeprecatedOutputs(t *testing.T) {
	...
	t.Run("deprecated output for --hcnet-core-db-url and --enable-captive-core-ingestion", func(t *testing.T) {
		originalStderr := os.Stderr
		r, w, _ := os.Pipe()
		os.Stderr = w
		stdLog.SetOutput(os.Stderr)
		...
		assert.Contains(t, string(outputBytes), "DEPRECATED - The usage of the flag --hcnet-core-db-url has been deprecated. "+
	...
	t.Run("deprecated output for env vars HCNET_CORE_DATABASE_URL and ENABLE_CAPTIVE_CORE_INGESTION", func(t *testing.T) {
		originalStderr := os.Stderr
		r, w, _ := os.Pipe()
		os.Stderr = w
		stdLog.SetOutput(os.Stderr)
		...
		testConfig := integration.GetTestConfig()
		testConfig.AuroraEnvironment = map[string]string{
			"HCNET_CORE_DATABASE_URL":     "temp-url",
